import { Mail, MapPin, Shield, FileText, DollarSign, CreditCard } from 'lucide-react';
...
export const Footer = () => {
  ...
  const handleCategoryClick = (category: string) => {
    ...
    setTimeout(() => {
      ...
      if (comparerSection) {
        comparerSection.scrollIntoView({
          behavior: 'smooth'
        });
      }
    }, 100);
  ...
  const categoryLinks = [{
    label: 'Remise en argent',
    category: 'cashback'
  }, {
    label: 'Cartes voyage',
    category: 'travel'
  }, {
    label: 'Sans frais annuels',
    category: 'no-fee'
  }, {
    label: 'Cartes étudiants',
    category: 'student'
  }, {
    label: 'Cartes premium',
    category: 'premium'
  }];
        ...
        <div className="grid md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-1">
            <Link to="/" className="flex items-center gap-2 mb-4 hover:opacity-80 transition-opacity">
              <CreditCard className="w-7 h-7 text-[#f7f7f7]" />
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-secondary-foreground/70">
              {categoryLinks.map(link => <li key={link.category}>
                  <button onClick={() => handleCategoryClick(link.category)} className="hover:text-secondary-foreground transition-colors text-left">
                    {link.label}
                  </button>
                </li>)}
            </ul>
